...
--3.2    
SELECT BusinessEntityID, FirstName, LastName    
FROM Person.Person    WHERE @ID = CASE @ID WHEN 0 THEN 0 ELSE BusinessEntityID END;       
GO       
--4    
...
FROM Sales.Customer;       
IF @Count < 500 
	PRINT 'The customer count is less than 500.';    
ELSE PRINT 
	'The customer count is 500 or more.';    
GO       
--2    
...
-- isting 12-15. Creating and Populating Table Variable 
...
FROM Person.Person AS P INNER JOIN Sales.Customer AS C    
	ON P.BusinessEntityID = C.PersonID;       
SELECT CustomerID, FirstName, LastName    FROM @myCustomers;   
...
DECLARE @ID INT;    DECLARE @Loc INT;       
--2    
...
CLOSE Territory;    DEALLOCATE Territory;       
